/**
 * Created by kaihui on 2018/6/30.
 */
//搜索：根据输入值，将拼音（汉字）转为汉字（拼音），然后同时搜索汉字和拼音。
var SimplePinYin = {
  enAry: [
  ...
  convertEN: function(cnStr) {//将汉字字符转成英文字母
    if (cnStr == null || cnStr.length == 0)
      return "";
    var tempStr = cnStr.charAt(0);
    if (cnStr.charCodeAt(0) <= 255)//如果是英文字母，直接返回
      return tempStr;

    //如果是汉字，则从中文字符串数组cnAry中查找对应字符串
    for (var i = 0; i < this.cnAry.length; i++) {
      if (this.cnAry[i].indexOf(tempStr) >= 0)//如果中文数组中存在这一项
        return this.enAry[i];//则返回汉字对应的英文字母
    }
    return '';
  },
  convertENs: function(str) {
    //var arr = str.split(' ');//将词组字符串以空格分隔成数组 "耳机 蓝牙"->[耳机,蓝牙]
    var arr = str.split('');//将词组字符串分隔成数组 "耳机 蓝牙"->[耳,机, ,蓝,牙]
    console.log("arr:"+arr);
    var arrPY = [],//arrPY对应拼音，arrPYS对应拼音首字母
      arrPYS = [],
      en;
    for (var i = 0; i < arr.length; i++) {
      en = this.convertEN(arr[i]);//对数组每一项转化成对应的英文字母
      if (en) {
        arrPY.push(en);
        console.log("arrPY:"+arrPY);
        arrPYS.push(en.charAt(0));
      }
    }
    return [arrPY.join(''), arrPYS.join(''), str];
  },
  isMatch: function(searchStr, pinyinArr) {
    // pinyinArr => ["zengzhiwei", "zzw", "曾志伟"]
    var isFind = false;
...
function query(str) {
  var str = str.trim();//去除字符串首尾空格
  if (!str) {
    return '';
  }

  var arr = SimplePinYin.convertENs(str);
  var res = arr.join(' | ');
  console.log(res);
}
// var str='hhh'
...
function search(list,str) {
  ...
  if(flag){//如果是常用词，则直接去商品列表的title中找
    ...
    arr.forEach((item,index)=>{
      if(item!==''){
        list.forEach((item)=>{
          if(item.title.indexOf(arr[index])>=0){
            arrPY.push(item);
          }
        })
      }
...
var result = segment.doSegment(text, {
  ...
  stripStopword: true//去除停止符，在分词时结果中的"之所以"和"因为"将被去除：
});
console.log(result);

let commonList=["手机","耳机","电脑","电视机"]
let ress=commonList.filter((item,index)=>{
  return index<3
})
console.log(ress);
